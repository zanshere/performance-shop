...
@section('title', 'Edit Produk - Admin MotorSpareParts')
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Produk</h2>
...
Sudut: 14°
Diameter: 120mm
Berat: 850 gram">{{ old('specifications', $specsText) }}</textarea>
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <select id="brand"
                                        ...
                                        @endforeach
                                        <option value="other" {{ old('brand', $product->brand) == 'other' ? 'selected' : '' }}>Lainnya</option>
                                    </select>
                                    @if(!in_array($product->brand, $brands->pluck('name')->toArray()))
                                        <input type="text"
                                               id="other_brand"
                                               name="other_brand"
                                               value="{{ old('other_brand', $product->brand) }}"
                                ...
                                <div>
                                    ...
                                    @if(!in_array($product->category, $categories->pluck('name')->toArray()))
                                        <input type="text"
                                               id="other_category"
                                               name="other_category"
                                               value="{{ old('other_category', $product->category) }}"
                                               class="input-field mt-2"
                                ...
                                <div>
                                    ...
                                    @php
                                        ...
                                        if ($product->compatibility) {
                                            $compatibility = json_decode($product->compatibility, true);
                                            $compatibilityText = implode(', ', $compatibility);
                                        }
                                    @endphp
                                    <input type="text"
                                           id="compatibility"
                                           name="compatibility"
                                           value="{{ old('compatibility', $compatibilityText) }}"
                                           class="input-field"
                                            ...
                                            <div class="relative">
                                                ...
                                                <div class="absolute top-0 right-0 flex space-x-1">
                                                    ...
                                                    <label for="remove-{{ $index }}"
                                                           class="bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center cursor-pointer"
                                                           title="Hapus">
    ...
    <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-md sm:w-full m-3 sm:mx-auto">
        <div class="card">
            <div class="p-6 text-center">
                ...
                <form id="delete-form" method="POST" action="{{ route('admin.products.destroy', $product->id) }}" class="space-y-3">
                    @csrf
                    @method('DELETE')
